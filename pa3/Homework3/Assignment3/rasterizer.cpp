...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    //    * v[i].w() 是顶点视图空间深度值 z。
    //    * Z 是当前像素的插值视图空间深度
    //    * zp 是z-buffer中zNear和zFar之间的深度，用于z缓冲
    ...
    for(int x=xmin;x<=xmax;x++)
    {
        ...
        {
            //判断是否在三角形内部
            if(insideTriangle(float(x+0.5),float(y+0.5),t.toVector4().data()))
            {
                //如果是，计算重心坐标，进行法向量、颜色等差值
                ...
                {
                    //每一个都是u= α⋅vert1[0]+β⋅vert2[0]+γ⋅vert3[0]/weight
    ...
    // 使用: 不要直接将三角形的颜色传递给帧缓冲，而是先将颜色传递给着色器以获取最终颜色；
